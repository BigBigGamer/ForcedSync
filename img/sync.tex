...
% \pgfplotstableread{data.tsv}\mytable 
    ...
    interface/.style={
		...
		% \draw (k) -- (text);
		\draw[-] (f1.south) --($(f1.south)+(0cm,-2em)$) -- node[below,pos=0.5] {Удержание} ($(f4.south)+(0cm,-2em)$) -- (f4.south);
		\draw[-] (f2.south) --($(f2.south)+(0cm,-1em)$) -- node[above,pos=0.5] {Захват} ($(f3.south)+(0cm,-1em)$) -- (f3.south);
